...
const ProductModal = ({ order, onClose, onUpdateStatus }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-white opacity-95 ">
      <div className="bg-white p-2 sm:p-6 rounded-lg shadow-lg  w-full sm:w-auto">
  ...
  setOpenDropdown(openDropdown === orderId ? null : orderId);
    ...
    useEffect(() => {
        const fetchAdminOrders = async () => {
            try {
                const response = await fetch('https://brasscraft-api.onrender.com/orders'); // Your API endpoint
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
      try {
        const response = await fetch(`https://brasscraft-api.onrender.com/orders/${orderId}`, {
          method: "PUT",
          headers: {
                ...
                <td className="p-3 relative">
                  ...
                  {openDropdown === order.id && (
                    <div className="absolute right-0 mt-2 w-32 bg-white shadow-md rounded-md border">
                      <ul>
                        <Link to=`/individualorder/${order.id} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">View</li>
                        <Link className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Edit</li>
                        <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Delete</li>
                      </ul>
                    </div>
                  )} */}

                  {/* <Link to={`/order/${order._id}`} className='flex items-center gap-2'>
                <p>view order</p>
                <FaRegEye /> */}
                  {/* </Link> */}
