...
export default function AddProduct() {
   ...
   if(images.length > 0 && images.length + formData.imageUrls.length < 7){
    try {
      ...
      const uploadPromises = images.map(async (image) => {
        ...
        formDataa.append("upload_preset", "cloudinary_2"); // Replace with your Cloudinary upload preset
  ...
  const handleSubmit = async (e) => {
   ...
   ;
      setLoading(true);
      setError(false);
      const res = await fetch('https://brasscraft-api.onrender.com/add-product', {
        method: 'POST',
        headers: {
